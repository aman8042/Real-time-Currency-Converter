...
#include <string>
#include <sstream>
#include <cstdlib>  // system() ke liye
#include <ctime>

using namespace std;

// Fetch exchange rates JSON and save locally on my file
bool fetch_exchange_rates() {
    int result = system("curl -s https://api.exchangerate-api.com/v4/latest/USD > currency.json");
    return (result == 0);
}

// parsing into the JSON file and retrieving the currency value according to the  currency code
...
// Convert from one currency to another using USD as base value (base value USD ko maan liya hai, pahle USD me convert hogafir waha se kisi aur me)
